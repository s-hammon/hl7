...
func TestUnmarshal_ORU(t *testing.T) {
	...
	require.Len(t, m.OrderGroup[0].Results, 39)
	require.Equal(t, "MAMMOGRAM DIGITAL SCREENING BILATERAL W/CAD AND DBT", m.OrderGroup[0].Results[0].ObservationValue)
	require.Equal(t, "Signed on 4/4/2025 3:25 PM by Julie M Farkas, M.D.", m.OrderGroup[0].Results[37].ObservationValue)
	require.Equal(t, "", m.OrderGroup[0].Results[38].ObservationValue)
	require.Equal(t, "O", m.PatientGroup.Visit.PV1.PatientClass)
	require.Equal(t, "JINKLEHEIMER", m.PatientGroup.PID.PatientName.GivenName)
	require.Equal(t, "19840526", m.PatientGroup.PID.DOB)
}
...
func TestUnmarshal_ORU_MultipleOrders(t *testing.T) {
	...
	err = Unmarshal(msg, &m)
	require.NoError(t, err)
	require.Len(t, m.OrderGroup, 2)
	t.Log(m.OrderGroup[0].OBR)
	...
	require.Len(t, m.OrderGroup[0].Results, 31)
	require.Len(t, m.OrderGroup[1].Results, 31)
}
